...
let accounts = process.env.tyys; //天翼云盘账号获取集
const serverChan = process.env.SENDKEY; //serverChan
const telegramBot = { //telegramBot
  key: process.env.WECOM_BOT_KEY || '',
  telphone: process.env.WECOM_BOT_TELPHONE || '',
};
const wecomBot = { //wecomBot
  key: process.env.WECOM_BOT_KEY || '',
  telphone: process.env.WECOM_BOT_TELPHONE || '',
};
//------------------------------------------------
...
const doTask = async () => {
  ...
  for (let index = 0; index < tasks.length; index += 1) {
    ...
    if (index === 0) {
      // 签到
      result.push(`${res.isSign ? '今天已经签到过了，' : ''}签到获得${res.netdiskBonus}M空间`);
...
const pushServerChan = (title, desp) => {
  if (!serverChan) { return; }
  const data = {
    title,
    desp,
  };
  superagent.post(`https://sctapi.ftqq.com/${serverChan}.send`)
    .type('form')
    .send(data)
    .end((err, res) => {
      if (err) {
...
async function main() {
  logger.log("开始天翼网盘签到");
  if(!accounts) {
    logger.log("没有获取到天翼云账号信息，请设置环境变量tyys");
    return;
  }
  accounts = accounts.split(/[\n ]/);
  for (let i = 0; i < accounts.length; i+=2) {
      const userNameInfo = mask(accounts[i], 3, 7);
      try {
        logger.log(`账户 ${userNameInfo}开始执行`);
        await doLogin(accounts[i], accounts[i+1]);
